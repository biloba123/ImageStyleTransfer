...
//         option.setWaterMarkOption(getWaterMarkOption(activity));
    ...
    public static void showPhotoEdit(Activity activity)
    {
        if (activity == null) return;
        // see-http://tusdk.com/docs/android/api/org/lasque/tusdk/impl/components/base/TuSdkHelperComponent.html
        final TuSdkHelperComponent componentHelper = new TuSdkHelperComponent(activity);

        TuAlbumComponent component = TuSdkGeeV1.albumCommponent(activity, new TuSdkComponent.TuSdkComponentDelegate()
        {
            @Override
    ...
    private static void openEditMultiple(TuSdkHelperComponent componentHelper, TuSdkResult result, Error error, TuFragment lastFragment)
    {
        ...
        {
            component = TuSdkGeeV1.editMultipleCommponent(componentHelper.activity(), delegate);
        }
        else
        {
